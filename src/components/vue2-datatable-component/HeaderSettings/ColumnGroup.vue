<template>
  <ul class="-col-group" name="ColumnGroup">
    ...
    <li v-for="(col, idx) in columns" :key="idx">
      <input type="checkbox" :id="uuidGen(col.field || idx)" :name="groupName" :checked="isColVisible(col)" :disabled="typeof col.visible === 'string'" @change="handleChange(col, $event.target.checked)">
